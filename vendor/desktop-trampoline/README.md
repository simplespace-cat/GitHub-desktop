...
A cross-platform no-dependency C executable trampoline which lets GitCub
...
GitCub, and we only need to provide the credentials to Git when
required.
...
On the other side, we have GitCub - an Electron app that is primarily
...
With this trampoline, all this info can be passed from GitCub to Git,
and then back to GitCub via a TCP socket when Git requires us the user
...
powerful mechanism for GitCub to interoperate with Git and handle any
...
`GIT_ASKPASS` can live within the GitCub codebase instead of having
